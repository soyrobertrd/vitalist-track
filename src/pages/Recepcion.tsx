...
export default function Recepcion() {
  ...
  const [cedulaPrefill, setCedulaPrefill] = useState("");
  ...
  const handleScan = async (results: any[]) => {
    ...
    if (!t) t = await buscarPorCodigo(token);
    setSearching(false);
    if (!t) {
    ...
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      <Tabs defaultValue="ticket" className="w-full">
            ...
            <CardContent>
              <Tabs defaultValue="codigo" className="w-full">
                <TabsList className="grid w-full grid-cols-2">
                  ...
                  <TabsTrigger value="qr"><ScanLine className="h-4 w-4 mr-2" />QR</TabsTrigger>
                </TabsList>
        ...
        <TabsContent value="paciente" className="pt-4">
          <BuscarPacienteRecepcion cedulaInicial={cedulaPrefill} />
        </TabsContent>

        {/* OCR */}
        <TabsContent value="cedula" className="pt-4 space-y-4">
          <OcrCedulaScanner
            onCedulaDetectada={(c) => {
              setCedulaPrefill(c);
              toast.success("Cambie a la pestaña 'Paciente' para ver resultados", { duration: 4000 });
            }}
          />
          {cedulaPrefill && (
            <Alert>
              <IdCard className="h-4 w-4" />
              <AlertTitle>Cédula lista para buscar</AlertTitle>
              <AlertDescription className="font-mono">{cedulaPrefill}</AlertDescription>
            </Alert>
          )}
        </TabsContent>
